...
void* handler_function(void* arg) {
	...
	NetworkRequestChannel* workChan[p->w];
	...
	for(int i = 0; i < p->w; ++ i )
	{
		workChan[i] = new NetworkRequestChannel(p->hostName, p->portNum);
	}
	//create fd_set
	fd_set rfds;
	FD_ZERO(&rfds);
	for(int i = 0; i < p->w; i++)
	{
		FD_SET(workChan[i]->socketFD(), &rfds);
		nfd = max(workChan[i]->socketFD(), nfd);
	}
	...
	while(rec < 3 * p->n)
	{
		...
		else
		{
			for (int i = 0; i < p->w; i++)
			{
				if(FD_ISSET(workChan[i]->socketFD(), &rfds))
				{
					//server response
					...
					if(sent < 3 * p->n)
					{
						...
						sentMsgs[i] = req;
						sent++;
						if(workChan[i]->socketFD() > nfd)
						{
							nfd = workChan[i]->socketFD();
							nfd++;
						}
					}
				}
...
int main(int argc, char * argv[]) {
	...
	int opt = 0;
	while ((opt = getopt(argc, argv, "n:b:w:m:p:h:e")) != -1) {
		switch (opt) {
			case 'n':
				n = atoi(optarg);
				break;
			...
			default:
				...
				std::cout << "Example: ./client_solution -n 10000 -b 50 -w 120 -p 6969 -h build.tamu.edu -m 2" << std::endl;
