...
class ReqBuffer { //buffer class that implements queue
	private:
	std::mutex buffLock;
	public:
	std::queue<std::string> bufferActual;
	ReqBuffer()
	{
	}
	std::string buffPop()
	{
		std::string ret;
		if(bufferActual.empty() == true)
		{
			//std::cout << "Queue is empty, can't pop." << endl << flush;
			return "Failed.";
		}
		else
		{
			buffLock.lock();
			ret = bufferActual.front();
			bufferActual.pop();
			buffLock.unlock();
			return ret;
		}
	...
	void buffPush(std::string in)
	{
		buffLock.lock();
		bufferActual.push(in);
		buffLock.unlock();
	}
};

struct PARAMS { //params to pass in between classes
    /*
    ...
    */
	...
	std::vector<int>* johnCount;
	std::vector<int>* janeCount;
	std::vector<int>* joeCount;
	std::mutex johnLock;
	std::mutex janeLock;
	std::mutex joeLock;
};
...
void* populateRequests(void* p) //helper method to populate requests used later on
{
	PARAMS* params = (PARAMS*) p;
	for(int i = 0; i < params->reqNum; i++)
	{
		std::string req = "data " + params->name;
		params->requestBuffer->buffPush(req);
	}
}
...
void* worker_thread_function(void* arg) {
    ...
    PARAMS* p = (PARAMS*)arg;
	p->uniLock.lock();
	std::string s = p->workChan->send_request("newthread");
	RequestChannel *workerChannel = new RequestChannel(s, RequestChannel::CLIENT_SIDE);
	p->uniLock.unlock();
    
    while(true) {
        std::string request = p->requestBuffer->buffPop();
        std::string response = workerChannel->send_request(request);
        
        if(request == "data John Smith") {
			p->johnLock.lock();
            p->johnCount->at(stoi(response) / 10) += 1;
			p->johnLock.unlock();
        }
        else if(request == "data Jane Smith") {
			p->janeLock.lock();
            p->janeCount->at(stoi(response) / 10) += 1;
			p->janeLock.unlock();
        }
        else if(request == "data Joe Smith") {
			p->joeLock.lock();
            p->joeCount->at(stoi(response) / 10) += 1;
			p->joeLock.unlock();
        }
...
int main(int argc, char * argv[]) {
    ...
    if(pid == 0){
		...
		john.threadNum = w;
		john.reqNum = n;
		john.requestBuffer = &reqBuff;
		john.johnCount = &john_frequency_count;
		john.janeCount = &jane_frequency_count;
		john.joeCount = &joe_frequency_count;
		john.workChan = chan;
		...
		jane.threadNum = w;
		jane.reqNum = n;
		jane.requestBuffer = &reqBuff;
		jane.johnCount = &john_frequency_count;
		jane.janeCount = &jane_frequency_count;
		jane.joeCount = &joe_frequency_count;
		jane.workChan = chan;
		...
		joe.threadNum = w;
		joe.reqNum = n;
		joe.requestBuffer = &reqBuff;
		joe.johnCount = &john_frequency_count;
		joe.janeCount = &jane_frequency_count;
		joe.joeCount = &joe_frequency_count;
		joe.workChan = chan;
		//for some reason you need to do this instead of making them pointers in the first place
		PARAMS* johnP = &john;
		PARAMS* janeP = &jane;
		PARAMS* joeP = &joe;
...
/*--------------------------------------------------------------------------*/
        ...
        fflush(NULL);
		pthread_t requestsThread[3];
		
		pthread_create(&requestsThread[0], NULL, populateRequests, (void*)johnP);
		pthread_create(&requestsThread[1], NULL, populateRequests, (void*)janeP);
		pthread_create(&requestsThread[2], NULL, populateRequests, (void*)joeP);
...
/*--------------------------------------------------------------------------*/
		...
		pthread_t responsesThread[w];
		
		for(int i = 0; i < w; i++)
		{
			pthread_create(&responsesThread[i], NULL, worker_thread_function, johnP);
		}
		for(int j = 0; j < w; j++)
		{
			pthread_join(responsesThread[j], NULL);
		}
